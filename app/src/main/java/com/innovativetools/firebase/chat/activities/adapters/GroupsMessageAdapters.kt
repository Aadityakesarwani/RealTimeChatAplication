...
class GroupsMessageAdapters(
        ...
        return if (viewType == MSG_TYPE_RIGHT) {
            val view =
                LayoutInflater.from(mContext)
                    .inflate(R.layout.list_group_chat_right, viewGroup, false)
            ViewHolder(view)
        } else {
            val view =
                LayoutInflater.from(mContext)
                    .inflate(R.layout.list_group_chat_left, viewGroup, false)
            ViewHolder(view)
        }
    }
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        val user = chat?.let { userList?.get(it.sender) }
        try {
        ...
        if (Utils.isEmpty(attachType)) {
            ...
            if (attachType.equals(IConstants.TYPE_TEXT, ignoreCase = true)) {
                            ...
                            if (File(receiverPath).exists()) {
                                ...
                                try {
                                    viewHolder.recordingPlayerView!!.txtProcess.text =
                                        chat?.attachmentSize?.let { Utils.getFileSize(it) }
                    ...
                    if (!Utils.isEmpty(user!!.getImageURL())) {
                        user.getImageURL()?.let {
                            Utils.setProfileImage(
                                mContext, it, viewHolder.recordingPlayerView!!.voiceUserImage
                            )
                        }
                    }
                } catch (e: Exception) {
                ...
                try {
                    ...
                    viewHolder.txtFileSize!!.text = chat?.attachmentSize?.let { Utils.getFileSize(it) }
                } catch (e: Exception) {
                    Utils.getErrors(e)
                }
                ...
                try {
                    ...
                    viewHolder.txtVideoSize!!.text = chat?.attachmentSize?.let {
                        Utils.getFileSize(
                            it
                        )
                    }
        ...
        try {
            val first = Utils.dateToMillis(
                mChats?.get(position - 1)?.datetime
            )
        ...
        when (viewHolder.itemViewType) {
            MSG_TYPE_LEFT -> try {
                assert(user != null)
                viewHolder.txtName.text = user!!.username
            } catch (e: Exception) {
                Utils.getErrors(e)
            }
            MSG_TYPE_RIGHT -> {}
